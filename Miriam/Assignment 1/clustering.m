...
while change > 10^-15
    for i = 1:length(amount_req)
        distc1(i) = distance([amount_req(i),case_duration(i),total_activities(i)],c1);
        distc2(i) = distance([amount_req(i),case_duration(i),total_activities(i)],c2);
        distc3(i) = distance([amount_req(i),case_duration(i),total_activities(i)],c3);
        
        
        if (distc1(i) == min([distc1(i),distc2(i),distc3(i)]))
            clust1x = [ clust1x , amount_req(i)];
            clust1y = [ clust1y , case_duration(i)];
            clust1z = [ clust1z , total_activities(i)];
        elseif(distc2(i) == min([distc1(i),distc2(i),distc3(i)]))
    ...
    c1new = centroid([clust1x;clust1y;clust1z]);
    c2new = centroid([clust2x;clust2y;clust2z]);
    c3new = centroid([clust3x;clust3y;clust3z]);
